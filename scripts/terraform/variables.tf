...
variable "ecs_volume_size" {
  description = " Set the system disk size ( requires at least 100G or larger. The default value is 100G). "
  nullable    = false
  type    = number
  default = 100
}
...
variable "Version" {
  ...
  validation {
    condition     = contains(["internlm2_5-1_8b-chat-arm-v1.0"], var.Version)
